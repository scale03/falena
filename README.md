#  Falena🦋

[![Build](https://github.com/scale03/falena/actions/workflows/ci.yml/badge.svg)](https://github.com/scale03/falena/actions)
[![Go Report Card](https://goreportcard.com/badge/github.com/scale03/falena)](https://goreportcard.com/report/github.com/scale03/falena)
[![License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)

**File Access Telemetry for Kubernetes via eBPF**

Falena is a next-generation observability agent built for Kubernetes. It leverages **eBPF (Extended Berkeley Packet Filter)** and **CO-RE (Compile Once, Run Everywhere)** principles to extract high-fidelity File Access telemetry directly from the Linux kernel.
